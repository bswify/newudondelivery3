...
class Apitest3Controller extends Controller
{
    /**
     * @inheritdoc
     */
    ...
    public function actionListtest3()
    {
...
//            print("<pre>" . print_r($item, true) . "</pre>");
            ...
            $sql2->select('*')->from('food')->where('IDRestaurant ='.$item['IDRestaurant'])->andWhere('MenuTypeName = "เมนูแนะนำ"');
...
//            $data11 = mysqli_query($con,$sql2);
            ...
            $sql2t->select('*')->from('food')->where('IDRestaurant ='.$item['IDRestaurant'])->andWhere('MenuTypeName = "เมนูธรรมดา"');

//            $sql2 = "SELECT * FROM food WHERE IDRestaurant = ".$row['IDRestaurant'];

            $command11t = $sql2t->createCommand();
            $data11t = $command11t->queryAll();

//            $data11 = mysqli_query($con,$sql2);

            foreach($data11t as  $item1t){
                $data2t =
                    array(
                        "IDFood" => $item1t['IDFood'],
                        "FoodImg" => $item1t['FoodImg'],
                        "FoodName" => $item1t['FoodName'],
                        "FoodPrice" => $item1t['FoodPrice'],
                        "IDFoodType" => $item1t['IDFoodType'],
                        "MenuTypeName" => $item1t['MenuTypeName'],
                        "IDRestaurant" => $item1t['IDRestaurant']
                    );
                $data3[] = $data2t;
            }
